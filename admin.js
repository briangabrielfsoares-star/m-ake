(() => {
  ...
  const CATEGORIES = window.ICANT_CATEGORIES || [
    { name: "Camisetas", slug: "camisetas" },
    { name: "Calças", slug: "calcas" },
    { name: "Calçados", slug: "calcados" },
    { name: "Acessórios", slug: "acessorios" }
  ];
  const CATS = CATEGORIES.map(cat => cat.slug);
  ...
  function categoryName(slug) { return CATEGORIES.find(cat => cat.slug === slug)?.name || slug; }

  function renderCategoryShells() {
    const sections = $('[data-category-sections]');
    if (sections && !sections.dataset.ready) {
      sections.innerHTML = CATEGORIES.map(cat => `
        <section id="${cat.slug}" class="product-section">
          <div class="section-title-row"><h2>${escapeHtml(cat.name).toUpperCase()}</h2><button class="text-link" data-filter="${cat.slug}">VER TUDO</button></div>
          <div class="product-grid" data-grid="${cat.slug}"></div>
        </section>`).join("");
      sections.dataset.ready = "true";
    }

    const mobileCats = $('[data-mobile-cats]');
    if (mobileCats && !mobileCats.dataset.ready) {
      mobileCats.innerHTML = CATEGORIES.map(cat => `<a href="#${cat.slug}" data-menu-link>${escapeHtml(cat.name).toUpperCase()} <span>›</span></a>`).join("");
      mobileCats.dataset.ready = "true";
    }
  }
  ...
  function openProduct(pid) {
    ...
    $('[data-product-detail]').innerHTML = `<div class="product-detail">
      ...
      <div class="product-detail-info">
        <span class="eyebrow">${escapeHtml(categoryName(p.category))}</span>
  ...
  document.addEventListener('click', (ev) => {
    const menuLink = ev.target.closest('[data-menu-link]');
    if (menuLink) $('[data-mobile-menu]')?.classList.remove('open');
    ...
    if (filter) { activeFilter = activeFilter === filter.dataset.filter ? null : filter.dataset.filter; renderProducts(); toast(activeFilter ? `Filtro: ${categoryName(activeFilter)}` : "Mostrando todos"); return; }
